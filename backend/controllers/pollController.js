...
exports.getVotedPolls = async (req, res) => {
  ...
  try {
    ...
    // Add `userHasVoted` flag for each poll
    const updatedPolls = polls.map((poll) => {
      const userHasVoted = poll.voters.includes(userId.toString());
    ...
    // Count total voted polls for pagination
...
exports.getPollById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching poll", error: err.message });
  }
};
...
exports.voteOnPoll = async (req, res) => {
  ...
  try {
    ...
    // Handle different poll types with if-else
